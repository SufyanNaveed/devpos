...
class Employee extends CI_Controller
{
    ...
    public function submit_user()
    {
        ...
        $config['upload_path']          = './userfiles/employee/';
        ...
        if($roleid_post == 6)
        {
         ...
         if($this->form_validation->run() == false && $roleid_post == 6){
            ...
            if ((string)$this->aauth->get_user($a)->id != $this->aauth->get_user()->id) {
                ...
                if ($nuid > 0) {
                    $res= $this->employee->add_employee($nuid, (string)$this->aauth->get_user($a)->username, $name, $roleid, $phone, $address, $city, $region, $country, $postbox, $location, $salary, $commission, $department,$whatsapp_link,$doc_image);
                    if($roleid_post == 6){ 
                        $d=$this->storeSpeciality($nuid,$speciality_data);
                        $avail=$this->storeAvailability($nuid,$this->input->post());
                    }
                    if($res == 1){
                        $this->session->set_flashdata('messagePr', 'Employee added Successfully!!');
