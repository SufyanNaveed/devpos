...
class Booking extends CI_Controller{
    ...
    public function getSpeciality($id)
    {
        ...
        $data['availability'] = $this->booking->getAvailability($id,'');
        $data['doctor_image'] = $this->booking->getDoctor_id($id);
    ...
    public function check_booking_availability()
    {
        ...
        // echo '<pre>';print_r($data);exit;
        $responce = $this->booking->check_booking_availability($data);
        if($responce){
            echo  json_encode(['status'=>200,'data'=>'Already booking exist on this time. Please selet another']);
        }else{
            echo json_encode(['status'=>404,'data'=>'not found']);
