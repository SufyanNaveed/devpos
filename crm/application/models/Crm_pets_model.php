...
class Crm_pets_model extends CI_Model
{
    ...
    public function getPets()
    {
        ...
        if($this->input->get('id')!=null ){
            ...
            $sql="SELECT p.pet_id id,p.pet_name,p.pet_gender,p.mark_difference ,p.date_of_birth,p.microchip_number ,p.pet_photo ,p.status ,
    ...
    public function addnew($pet_name, $pet_gender,$pet_color, $pet_breed, $pet_type, $microchip_number, $mark_difference,$date_of_birth,$pet_photo,$customer_id=0)
    {
        // echo "yes";exit;
    ...
    public function addnew_array($data,$customer_id=0)
    {
        foreach ($data as $key => $row) {
            $this->db->insert('pos_pets', $row);                
            $pet_id=$this->db->insert_id();
            $data=$this->session->userdata();
            // $customerId=$data['user_details'][0]->user_id;
            $customerId = $customer_id > 0 ? $customer_id : $this->session->userdata('user_details')[0]->cid;
