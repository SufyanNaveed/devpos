...
$('#doctor').change(function(){
    ...
    $.ajax({
        ...
        if(data.status==404){
            ...
            data.data.speciality.map(function(item){
                $("#speciality").append(new Option(item.name, item.id));
            })

            data.data.doctor_image.map(function(item){
                $("#doctor_image").empty();
                $("#doctor_image").append('<img alt="Doctor Image" src="http://54.226.232.117/devpos/userfiles/employee/'+item.picture+'" style="height: 100px !important;">');
            })

            $("#availslot").empty();
            var html = '';
            data.data.availability.map(function(item){
                // var html ='';
                // if(item.time == 'morning' && item.day == 'monday') {
                //     html +='<p style=" margin: 0; padding: 0;"><strong>Morning Available Slots</strong></p>';
                // } else if(item.time == 'evening' && item.day == 'monday'){
                //     html +='<p style=" margin: 0; padding: 0;"><strong>Evening Available Slots</strong></p>';

                // }
                // html +='<span>&nbsp;'+item.day.charAt(0).toUpperCase() + item.day.slice(1)+'</span><span> -- '+item.from+' - '+ item.to +'</span><br/>';                
                
                if(item.time == 'morning' && item.day == 'monday') {
                    html += '<b>Mon-Fri</b> -- '+item.from;
                }
                if(item.time == 'evening' && item.day == 'friday') {
                    html += ' - '+item.to+'<br/>';
                }

            })
            html += '<b>Sat-Sun</b> -- Not Available';

            $("#availslot").append(html);
        }
            ...
            data.data.map(itm => {
                itm.slots.map(time=>{
                    $('.times-list').append(
                        ...
                        <button type="button" data-time-from="`+ time.from+`" data-time-to="`+time.to+`" class="btn times-block-btn check_booking_availability">`+time.from+` - `+time.to+`</button>
                        </div>`:
                        `<div class="col-12 mb-3">
...
$('.availslot').on('click', 'button', function() {
    
    var doctorId=$("#doctor").val();
    var from= $(this).attr('data-time-from');
    var to=$(this).attr('data-time-to');
    var date=$(".date").val();
    
    $.ajax({
        url: "<?php echo base_url('booking/check_booking_availability/') ?>",
        method: "POST",
        data: {doctor_id:doctorId,from:from,to:to,on:date,<?php echo $this->security->get_csrf_token_name();?> : '<?php echo $this->security->get_csrf_hash();?>'},
    })
    .done(function(result) {
        var data=JSON.parse(result);
        $("#error-msg").html('');
        if(data.status==200){
            $("#error-msg").text('Already booking exist on this time. Please select another slot!');
            $('.book-now').hide();
        }else{
            $("#error-msg").text('');
            $('.book-now').show();
