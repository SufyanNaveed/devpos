...
class User extends CI_Controller
{
    ...
    //                 $this->db->insert('pos_pet_breeds', $data);
    //             // }
    //             $c = $c + 1;
    //         }
    ...
    public function get_breed()
    {
        ...
        $res = $this->db->get('pos_pet_breeds')->result_array();
    ...
    public function registerCustomer()
    {
		...
		$validation->set_rules('email', 'User Email', 'required|xss_clean|trim|is_unique[users.email]');
        ...
        if ($validation->run() == FALSE) {
			if($this->input->post('email') == '' || $this->input->post('phone') == '' || $this->input->post('password') == ''){
                $this->session->set_flashdata('messagePr', 'Email, Password and Phone are required!');
            }else{
                $this->session->set_flashdata('messagePr', 'Sorry! This email already exists.');
            }
            ...
            for($i=0; $i < $counter; $i++){
                ...
                $pet_photo = $data['totalFiles'][$i];

                $this->db->insert('pos_pet_color', ['title'=>$pet_color[$i]]);
                $pet_colorId = $this->db->insert_id();

                $data[] = array(
                    'pet_name' => $pet_name[$i],
                    'pet_gender' => $pet_gender[$i],
                    'microchip_number' => $microchip_number[$i],
                    'pet_color' => $pet_colorId,
                    'pet_breed' => $breed[$i],
                    'pet_type' => $type_id,
                    'mark_difference'=>$mark_difference[$i],
            ...
            $this->pets->addnew_array($data, $id);
